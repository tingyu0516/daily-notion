...
你(飞书) ── Hermes Agent ──┬── daily-notion MCP server ── Notion(每日任务/每日日志)
                           └── cron 8:55 / 14:00 / 22:00 / 23:00 (条件触发)
```
...
用 `hermes cron create` 建 4 个任务，时间与触发词如下。**prompt 第一句必须点名 skill 流程，并附上执行条件**（skill 里有完整条件逻辑，prompt 只需把任务指到对应流程）：

| 时间 | cron 表达式 | prompt |
|---|---|---|
| 08:55 | `55 8 * * *` | 执行 daily-notion skill 的【早报】流程，严格遵守其中的 cron 静默条件 |
| 14:00 | `0 14 * * *` | 执行 daily-notion skill 的【中途提醒】流程，严格遵守其中的 cron 静默条件 |
| 22:00 | `0 22 * * *` | 执行 daily-notion skill 的【复盘提醒】流程，严格遵守其中的 cron 静默条件 |
| 23:00 | `0 23 * * *` | 执行 daily-notion skill 的【深夜兜底】流程，严格遵守其中的 cron 静默条件 |
...
- 什么都不说 —— 22:00 一句轻提醒；23:00 未确认任务自动顺延，次日早报点名
