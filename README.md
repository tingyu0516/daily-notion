...
- 大任务不用自己拆 —— 口述大目标，它会问清产出和截止，给出 2-5 条带估时的拆解提案，确认后分布到合适的日期；同一天排不下会主动往后推
- 什么都不说 —— 复盘提醒时刻一句轻提醒；兜底时刻未确认任务自动顺延，次日早报点名

**冷启动**：首次使用时库里全空，早报 cron 会主动发一次初始化询问——口述你近几天的任务即可启动每日循环；回复"停止日报"则永久静默。库里有过任务之后，无任务的日子一律静默（休息日不打扰）。
